...
void uninitialisedYPointTest() {
    ...
    int result;               // The actual result of the function
    ...
    if (result == expectedResult) {
        printf("uninitialisedYPointTest test succeeded! (^ Expected failure)\n");
    } else {
...
/* This test will confirm that the yScaling function correctly computes and sets
 * the Y value for points when given a Y scale value of 0. The expected
 * results for all of the Y point values set in the initialisePoints function
 * are 0. */
void testZeroScale() {
